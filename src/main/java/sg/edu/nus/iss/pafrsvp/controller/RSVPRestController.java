...
public class RSVPRestController {
    ...
    public ResponseEntity<String> saveRSVP(@RequestBody String json) {
        try {
            ...
            RSVP result = this.svc.insertRSVP(r);
            JsonObject okResp = Json.createObjectBuilder().add("rsvpId", result.getId()).build();
            return ResponseEntity.status(HttpStatus.CREATED).contentType(MediaType.APPLICATION_JSON).body(okResp.toString());
    ...
    public ResponseEntity<String> aggregateByFoodType() {
        List<AggregationRSVP> ll = svc.aggregateByFoodType();
        JsonArrayBuilder b = Json.createArrayBuilder();
        for (AggregationRSVP l : ll) {
            b.add(l.toJson());
        }
        JsonArray jArr = b.build();
        return ResponseEntity.status(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON).body(jArr.toString());
    }
}
